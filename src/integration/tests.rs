...
#[test]
fn cursor_current_version_integration_status_is_current() {
    let _lock = integration_env_lock();
    let base = unique_base();
    let cursor_dir = base.join(".cursor");
    fs::create_dir_all(&cursor_dir).unwrap();
    let hook_path = cursor_dir.join(CURSOR_HOOK_INSTALL_NAME);
    // Derive the marker from the constant so a version bump does not silently
    // turn this into an assertion about a stale install.
    fs::write(
        &hook_path,
        format!(
            "#!/bin/sh\n# HERDR_INTEGRATION_ID=cursor\n# HERDR_INTEGRATION_VERSION={CURSOR_INTEGRATION_VERSION}\n"
        ),
    )
    .unwrap();
    std::env::set_var(CURSOR_CONFIG_DIR_ENV_VAR, &cursor_dir);
...
/// A hook installed before this fork renumbered its integration assets carries a
/// lower marker than the bundled asset, so it must reinstall rather than be
/// treated as current.
#[test]
fn cursor_pre_renumber_integration_status_is_outdated() {
    let _lock = integration_env_lock();
    let base = unique_base();
    let cursor_dir = base.join(".cursor");
    fs::create_dir_all(&cursor_dir).unwrap();
    let hook_path = cursor_dir.join(CURSOR_HOOK_INSTALL_NAME);
    let stale = CURSOR_INTEGRATION_VERSION - 1;
    fs::write(
        &hook_path,
        format!("#!/bin/sh\n# HERDR_INTEGRATION_ID=cursor\n# HERDR_INTEGRATION_VERSION={stale}\n"),
    )
    .unwrap();
    std::env::set_var(CURSOR_CONFIG_DIR_ENV_VAR, &cursor_dir);
    ...
    let cursor = statuses
        ...
        .find(|status| status.target == crate::api::schema::IntegrationTarget::Cursor)
        .expect("cursor integration status");
    assert_eq!(cursor.installed_version, Some(stale));
