...
pub enum StatusIndicatorStyle {
    /// The single-cell ASCII alphabet (`!` `>` `-`). Default here rather than
    /// upstream's `dots` because it is one cell in every terminal and in every
    /// font, and shares no ink between states — see `ui::status::state_mark`.
    #[default]
    Ascii,
    Dots,
    Symbols,
...
pub struct KeysConfig {
    ...
    /// Create a scratch workspace: a plain shell at $HOME, detached from the
    /// current workspace's Git identity so it never groups under another
    /// workspace's worktree in the sidebar. Default: "prefix+shift+c"
    pub new_scratch_workspace: BindingConfig,
    ...
    /// Cycle the focused tab through the built-in pane arrangements. Default: "prefix+space"
    pub cycle_layout: BindingConfig,
...
pub(crate) struct KeysConfigOverlay {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    new_scratch_workspace: Option<BindingConfig>,
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    cycle_layout: Option<BindingConfig>,
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl<'de> Deserialize<'de> for KeysConfig {
        ...
        D: Deserializer<'de>,
    {
        ...
        apply_field!(new_scratch_workspace);
...
impl KeysConfig {
    ...
    pub(crate) fn local_profile(&self, keybinds: &Keybinds) -> KeysConfigOverlay {
        ...
        copy_effective_action_field!(new_scratch_workspace, keybinds.new_scratch_workspace);
        ...
        copy_effective_action_field!(cycle_layout, keybinds.cycle_layout);
...
impl Default for KeysConfig {
    fn default() -> Self {
        Self {
            ...
            new_scratch_workspace: BindingConfig::one("prefix+shift+c"),
            ...
            cycle_layout: BindingConfig::one("prefix+space"),
