...
fn render_switch_button(app: &AppState, frame: &mut Frame, area: Rect) {
    ...
    if global_agent_counts(app).blocked > 0 {
        ...
        let (symbol, style) = state_icon(AgentState::Blocked, false, app.status_indicators, p);
        ...
        match entry {
            ...
            WorkspaceListEntry::Agent { entry_idx, .. } => {
                if let Some(agent) = agents.get(*entry_idx) {
                    ...
                    let (icon, icon_style) =
                        state_icon(agent.state, agent.seen, app.status_indicators, p);
                    let (prefix, detail_prefix, prefix_width) = mobile_tree_prefix(entry, bg, p);
    ...
    if let Some(ws) = app.active.and_then(|idx| app.workspaces.get(idx)) {
        ...
        for (idx, tab) in ws.tabs.iter().enumerate() {
            ...
            if app.show_tab_state_dots.enabled(true) {
                ...
                let (dot, dot_style) = state_icon(tab_state, tab_seen, app.status_indicators, p);
...
fn agent_summary_text(
    ...
    let symbol = match indicator_style {
        // `None` here means this segment carries no mark at all, which is a
        // decision about the segment rather than about the alphabet — so ascii
        // honours it too and only substitutes its own glyph where one belongs.
        StatusIndicatorStyle::Ascii => {
            dot_style_symbol.map(|_| state_icon_symbol(state, seen, indicator_style))
        }
...
mod tests {
    ...
    fn agent_summary_leads_with_attention_states_in_priority_order() {
        ...
        let segments = agent_summary_segments(counts, StatusIndicatorStyle::Ascii);
    ...
    fn agent_summary_hides_empty_categories() {
        ...
        let labels: Vec<String> = agent_summary_segments(counts, StatusIndicatorStyle::Ascii)
            .into_iter()
            .map(|(text, _)| text)
            .collect();
        assert_eq!(
    ...
    fn agent_summary_collapses_to_all_idle_without_attention() {
        ...
        assert_eq!(
            agent_summary_segments(counts, StatusIndicatorStyle::Ascii),
    ...
    fn agent_summary_reports_no_agents_when_empty() {
        assert_eq!(
            agent_summary_segments(GlobalAgentCounts::default(), StatusIndicatorStyle::Ascii,),
