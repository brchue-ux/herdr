...
- The sidebar can draw a fleet signal bar on its reserved header row, above the first Space in the tree: eight fixed slots — `review`, `ask`, `report`, `busy`, `stopped`, `dirty`, `push`, `pr` — saying what the fleet is waiting on. Every slot is drawn in every frame, named and grey while its signal is quiet and coloured and animated while it is live, so the resting bar is the legend for the alerting one. Nothing here is a new measurement: the slots read the agent states the detector already publishes, the `summary` tokens workers already report, the work-volume level Herdr already samples, and the uncommitted-work and pull-request counts the sidebar counters already cache. Off by default under `[ui.sidebar.notifications]`, because three of the slots read counts that cost a `git status` scan or a request to the forge and Herdr only pays for those while something is drawing them. As the panel narrows the bar gives up the names, then the gaps between marks, and finally draws nothing rather than a prefix of itself — it never hides a slot, because a bar missing three of eight signals cannot be read at all.
