...
- A new `abyss` built-in theme (`[theme] name = "abyss"`) reproduces a near-black void, cyan primary accent, amber secondary accent, and a green "ok" status color, matching the "Rio Window, Assembled" mockup's own palette exactly rather than an approximation of it. It is additive — the existing default (`catppuccin`) is unchanged, so nothing about an existing config changes on upgrade. The diff/Changes zone's own width, previously a fixed 100 columns regardless of how much room it had, is now a percentage of the space left after the sidebar, so it keeps roughly the same Terminal:Changes ratio the mockup uses instead of shrinking to an ever-smaller sliver on a wider screen; `ui.diff_zone_width_threshold` still gates whether the zone shows at all, unchanged. The zone also scrolls on its own now — a mouse wheel notch over it moves `diff_pane_scroll` independently of the sidebar's and the pane's own scrollback, clamped to the diff's actual length — where previously it had no scroll state at all and always rendered from the top.
