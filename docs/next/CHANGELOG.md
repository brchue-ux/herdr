...
### Added
- A Windows `herdr --remote` client can now paste a clipboard image straight into a remote agent's pane, the same way a Linux or macOS client already could. The Windows half of that bridge was never implemented: `platform::read_clipboard_image` returned `None` on Windows, and the whole bridging block lived in the Unix raw-stdin arm of the client loop, which Windows does not use — its input path hands the loop semantic events instead. Both halves are now there. Windows reads the clipboard's registered `PNG` format first, so a screenshot keeps its own encoding and alpha, and falls back to `CF_DIBV5` then `CF_DIB`, converting the bitmap to PNG in Herdr rather than shelling out to a helper the way the Linux and macOS readers do (24-bit and 32-bit, `BI_RGB`/`BI_BITFIELDS`/`BI_ALPHABITFIELDS`, masks stored either inside the header or after it, top-down and bottom-up). The client's trigger is recognised on the semantic-event path too: an empty bracketed paste, which is what a terminal delivers when the clipboard holds something that is not text, or a press of the `keys.remote_image_paste` combo. Dropping one image file onto the terminal window works over the same path, since a drop arrives as a paste of the file's path. The remote server end is unchanged — it already staged the bytes to a temp file and pasted that path into the focused pane. Note that `keys.remote_image_paste` defaults to `ctrl+v` and Windows Terminal takes `ctrl+v` for its own paste before Herdr sees it, so a Windows user should rebind the trigger to something it does not claim, such as `alt+v`. Clipboard data is untrusted input, so the bitmap decoder bounds-checks every read, does all its length arithmetic checked, caps dimensions at 16384px and 16 megapixels before sizing any allocation from them, refuses channel masks that are not disjoint contiguous runs, and encodes through a writer that fails rather than growing past the protocol's payload limit.
...
- The persistent background scene is a three-quarter view of a real system rather than a flat map of one, and every body is captioned. Second mates were seated on a **single** orbit at one radius, so eight of them read as a clock face: the ladder is now twelve distinct radii spanning 6.13x from innermost to outermost, seated in roster order, with the outermost running off the frame edge rather than inscribed in it. Twelve rather than eight because eight seated eight of the eleven Spaces a real fleet has and silently drew none of the other three; the extra rungs are cut into the ladder's own four widest gaps, so every radius the scene already used is still a radius and only the outer half of the ring tightens. Where each mate starts on its rung, how far its own orbital plane is tilted out of the shared one, and where its periapsis falls are all read off its seat number rather than off its rung's radius, which is what keeps the ring spread at any capacity: read off the radius, twelve mates piled nine-deep into one third of the sky with their captions on top of each other. The plane is foreshortened, each mate's orbit is tilted a little out of it and is an ellipse whose eccentricity comes from that project's own file count, and a body on the near side of its orbit draws larger and passes in front of one on the far side. The sun sits off-centre, right of the strip the worker tree occupies, instead of dead in the middle of the window. Each mate now also carries a caption that follows it — its name, its file count and its streak, in cold cyan with no border or box — fading out where it would cross the panel or the sun's own caption rather than being cut. The overflow disclosure now names its key in the frame — `5 of 17 mates dropped · smallest by files at HEAD` — where it was a countable fan of dots that said how many were missing and nothing about which, or why. The scene's own registers are unchanged: the same file count still sets a body's radius, the same streak still sets its ring, and every rung is still filled by size.
...
- A code-diff pane can now sit to the right of the terminal, showing the running `+`/`-` edits the coding agent in the focused pane has made this session — not that pane's `git diff`. It reads a per-pane agent-edit log rather than any checkout, so git status has stopped being the gate: a pane that is not a git worktree at all still shows its agent's work, uncommitted changes that were already there before the agent started are no longer mixed in with what it did, and a pane whose agent has not touched anything shows "no edits yet" rather than a message about the checkout. Edits arrive over a new `pane.report_edit_diff` socket method (`herdr pane report-edit-diff` on the CLI door): one cumulative unified diff per file, recomputed against that file's session-start content every time and **replacing** — never appended to — that file's previous entry, so three edits to one file read as one clean diff instead of three stacked hunks. An empty diff clears one file, `--clear-all` empties the pane's whole log, and each report — one file's diff — is capped at 4,000 lines, the same ceiling the parser reads to; the pane's log across all its files has no total ceiling. The bundled Claude Code integration sends that `--clear-all` on `SessionStart`, so a fresh session never inherits the last one's edits, and reaps stale on-disk baseline snapshots on Unix while it is there. Herdr does not capture the edits themselves, though, and installs nothing that does: that is a `PreToolUse`/`PostToolUse` hook pair you write and drop in beside the managed hook, so the zone stays on "no edits yet" until you do. Claude Code is the only agent wired for it so far. The zone's own mechanics are unchanged: it is a fixed third zone, sidebar and terminal untouched, `ui.diff_zone_width_threshold` (default 300) gates it on remaining width after the sidebar, same mechanism as `mobile_width_threshold`, and below that width it folds to a popup-overlay fallback opened with `toggle_diff_pane` (default `prefix+d`) so the changes are never simply gone. It follows whichever pane is focused, not always a Space's first tab: a Space can hold several workers, each its own tab, and switching between them changes what the zone shows without disturbing the Space's own sidebar label or branch. Each file opens under a ruled card header instead of a raw `diff --git` line — synthesized from the path the report names, since the `diff -u` output a hook has on a pipe carries no such line — hunks cut across as a tinted rule rather than dim text, and every content row carries the same two-column old/new line-number gutter and rail marker the sidebar tree's own connectors use, with added/removed rows washed in a tint of the theme's green/red rather than only their text recolored.
...
- Herdr's default theme is now `abyss`, a new built-in reproducing the "Rio Window, Assembled" mockup's own palette exactly (near-black void, cyan primary accent, amber secondary accent, a green "ok" status color) rather than an approximation of it. It replaces `catppuccin` as the theme a fresh install and any existing config with no explicit `[theme] name` resolve to, including as the `auto_switch` dark-appearance fallback; an existing config that already sets `[theme] name` (or `dark_name`/`light_name`) is completely unaffected, since only the unconfigured default changed. Set `[theme] name = "catppuccin"` to keep the previous look. Unlike every other built-in theme, `abyss` also paints its own sidebar background — the mockup's `--void` near-black — instead of leaving it as `Color::Reset` to inherit whatever the host terminal's own background happens to be; the original ship left it unset as a host-contrast approximation, but on a host whose own default background is not already near-black (a light theme, a mid-grey terminal profile) that let an arbitrary color show straight through the sidebar in place of the mockup's own backdrop, with the card glow and accents reading flat against it rather than lit. The diff/Changes zone's own width, previously a fixed 100 columns regardless of how much room it had, is now a percentage of the space left after the sidebar, so it keeps roughly the same Terminal:Changes ratio the mockup uses instead of shrinking to an ever-smaller sliver on a wider screen; `ui.diff_zone_width_threshold` still gates whether the zone shows at all, unchanged. The zone also scrolls on its own now — a mouse wheel notch over it moves `diff_pane_scroll` independently of the sidebar's and the pane's own scrollback, clamped to the diff's actual length — where previously it had no scroll state at all and always rendered from the top.
...
### Fixed
- A worker no longer draws a bordered card of its own beside the mate that dispatched it. It is drawn *inside* that mate's card, under a dashed rule, as one compact row: its mark, its name and its dim status line. The behaviour already existed and could only ever be seen on the pixel-card path — every gate on it resolved through `image_card::card_covers_row`, so a session with `kitty_graphics` off, which is the default and is what the character card at 34 columns and up renders, drew a sub-agent card per worker, branch-connected to its mate but standing beside it. The fold is now a fact about the shell rather than about pixels, and both renderers honour it. Which card a worker lands in is unchanged and was always general: the nearest Space up its own ownership chain. So a second mate's own workers fold into the second mate's card, not the first mate's, and a worker a worker dispatched is stepped in one fixed column inside whichever card holds it — one step whichever chain it came down. A mate itself is untouched: a second mate is still its own card, still branch-nested under whoever owns it, at its own rung of the width ladder. Its box now grows down over the workers inside it and closes under the last one; a list the panel runs out of room for stops one worker short of the floor so the closing rule is still a rule rather than a worker's name with dashes either side of it. A worker keeps everything a row of its own carried except the border and the connector — its command-acknowledgement flashes and its failure marker follow it inside the box, the marker riding its own row rather than climbing a border it no longer has. Below 34 columns, where the panel draws no boxes at all, nothing changes: a worker is still its own line with its own connector and its own air around it. Scrolling the panel does not take a worker out of the card either. Which card a worker is in is ownership and nothing else, so a mate whose own row has scrolled off the top of the panel keeps its workers: the first of them still on screen draws that mate's box in its place, at the mate's own columns and in the mate's own colour, opened at the top rather than closed there — the sides run out of the panel with the same dashed seam between them that separates a mate from its crew, and on the pixel path the box's top edge and both its corners are simply above the panel and never drawn. Until now a worker took a card of its own back the moment its mate's row left the screen, which broke one mate's list into a stack of separate boxes with their borders touching, and relaid the whole tree while the reader was scrolling through it.
- A Claude Code pane's three zones no longer flicker apart while the agent is drawing. Herdr holds a pane's last complete frame through a DEC 2026 synchronized update, a resize's own redraw, and an alternate-screen history harvest, so a half-drawn or mid-scroll grid is never painted. The hold freezes the retained copy the pane's rows are drawn from, but not the live grid — and the Claude split's zone boundaries were read from the live grid, so every frame that landed inside a hold split frozen rows at a live screen's boundaries. Claude Code repaints its whole screen inside a synchronized update, so mid-batch the live grid is a torn screen with no recognizable composer box in it: the split disengaged for that frame, the transcript and composer jumped down by the eight rows the command-log zone had taken, and the zone itself vanished, all of it back on the next frame. Measured against a real terminal, sixteen of forty samples caught a focused Claude pane mid-jump while the agent was repainting. A live grid that is still Claude-shaped but a different shape — a prompt that wrapped onto a second composer row — moved the split instead of dropping it, which put Herdr's own dividers a row away from the agent's rules that were still on screen, and an alternate-screen harvest scrolls the same grid on purpose under the same promise. The split a held frame draws is now the split the last unheld paint recorded, which is the one that describes those rows; it is refused only when the pane has since become too short to draw it into. This also fixes a mouse-selection defect the hold made intermittent: a selection stores a grid row and its highlight is drawn back through whatever split the frame resolved, so a frame that dropped the split drew the same selection eight rows away from the text it was taken from, and back on the next.
- A click, drag, hover or wheel notch in a Claude Code pane is no longer reported to the agent eight rows off. The pane's transcript and composer are drawn above the grid rows they were copied from, by however many rows the command-log zone took. Everything that reads a drawn row projects through that shift — the selection anchor, the drag, the highlight, the typing caret, clickable hyperlink targets — except what Herdr told the pane's own app a mouse event landed on, which was sent as if a drawn row were the grid row of the same index. Claude Code turns basic mouse tracking on for its own clickable UI, so with reporting live every reported mouse position in the pane was wrong by exactly that shift. The command-log zone also stopped being reachable by the wheel while reporting was on: those rows are Herdr's own, with no grid row under them, but the pane's app was offered the notch first and claimed every one of them. The zone now takes its own notch first, and declines everything outside its own rows.
- A `herdr --remote` session started from the Windows binary now notices a dead link in about a minute instead of waiting on TCP. Herdr's managed SSH config exists to add fallback `ServerAliveInterval`/`ServerAliveCountMax` values when the user has set none, and it was disabled outright on Windows because its other job — a private per-attach OpenSSH control socket — needs `ControlMaster`, which Windows OpenSSH does not have. Losing the whole config to that lost the keepalives too, and OpenSSH's own default is `ServerAliveInterval 0`: a Windows remote attach whose connection died silently, from a sleeping laptop, dropped wifi or a VPN cutting out, was not detected until TCP itself gave up, potentially for hours. The control socket is now optional rather than the config's precondition, so Windows gets the same generated config with the keepalive fallback and no `ControlMaster`. It `Include`s `%USERPROFILE%\.ssh\config` and `%PROGRAMDATA%\ssh\ssh_config` first, exactly as the Unix config includes `~/.ssh/config` and `/etc/ssh/ssh_config`, so ssh's first-value-wins rule still keeps any `ServerAlive*` the user set, including an explicit `0` to keep them off, and no existing config file is read differently or written to. Include paths are rendered with forward slashes because OpenSSH's config parser does not resolve a path carrying backslashes and reports nothing when it fails to — a dropped `Include` would have left Herdr's fallback overriding the user's own settings instead of standing behind them. macOS and Linux behaviour is unchanged. `[remote].manage_ssh_config`, documented until now as having no effect on Windows, is live there.
...
- A `pane read`/`agent read --source recent` against an idle alternate-screen agent no longer slows the whole window down while it runs. Holding the harvested pane's presented frame (above) keeps the harvest's own scroll off the screen, but the pane still asked for a render on every chunk of output that scroll produced — and that ask is not pane-scoped. With Kitty graphics on, a non-empty graphics cache, or the UI in any mode but plain terminal, herdr cannot take its per-pane retained-frame path and recomposites the entire window instead, so a fleet-status watcher polling one pane bought the whole UI seconds of full recomposites per poll — every one of them drawing a frame identical to the last, because the single pane whose content had changed was the one being held. A pane whose frame is held for a harvest now requests no render at all, the same way an open synchronized update already suppresses one, and is marked dirty once when the hold lifts. Measured on a fifteen-pane session: the harvest's render requests fall from 24/s to none and the frame traffic it added — 29% above idle — to none, with the harvested text byte-for-byte unchanged.
