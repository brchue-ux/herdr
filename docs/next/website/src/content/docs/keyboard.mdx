...
Herdr is mouse-native. You can click panes, tabs, workspaces, and agents, drag split borders, right-click a pane to paste, and open context menus with `shift`+right-click, all without learning a single keybinding. Keyboard control is an optional layer, not a requirement.
...
| Close pane | `prefix+x` |
| Cycle built-in pane arrangements | `prefix+space` |
...
| New scratch workspace | `prefix+shift+c` |
