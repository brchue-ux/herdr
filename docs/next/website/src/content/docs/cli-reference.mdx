...
Each call replaces that file's previous entry; it never appends. Snapshot each file's content once, the first time the agent touches it in a session, and recompute the whole diff against that same fixed baseline on every later edit, so three edits to one file stay one clean diff instead of three stacked hunks. `--file` is the path as you want it displayed; it is not resolved or checked against the pane's cwd. The diff does not need a `diff --git` line — Herdr synthesizes one from `--file` so each reported file opens under its own header card.
...
A diff longer than 4000 lines — the same ceiling the Changes zone renders to — is rejected whole rather than stored truncated. The log is display-only, takes no lifecycle authority, and is not restored after a server restart.
