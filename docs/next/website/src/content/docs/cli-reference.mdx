...
herdr server
herdr server stop
herdr server swap --exe <path> [--dry-run] [--yes] [--allow-downgrade] [--promote-client <path>|--no-promote-client]
...
`herdr server` runs the headless server explicitly. Use it for supervised or service-style setups. `swap` live-swaps the running server onto a locally built binary, checking protocol compatibility and the workspace inventory before and after the handoff; see [Session state](/docs/session-state/#swapping-onto-a-locally-built-binary). `reload-config` applies reloadable settings without restarting panes. `agent-manifests` shows the active agent detection manifest sources, cached remote versions, and last remote update results. `update-agent-manifests` fetches remote manifest updates immediately, reloads them into the running server, and prints the updated manifest status; pass `--json` for the raw status response. `reload-agent-manifests` reloads agent detection manifests into the running server after local override edits.

## Background scene

```bash
herdr background on
herdr background off
herdr background status [--json]
```

Turns the persistent whole-terminal background scene on or off, and reports why it is not drawing when it is not.

The scene draws the fleet as an orrery behind every pane — the root of each owner tree as a sun, its second-level workspaces as planets, their workers as moons — mirroring the same tree the sidebar draws.

A project's body is drawn at its real size. Herdr measures nothing itself: it reads a `files` workspace metadata token — the project's tracked files at `HEAD`, as a plain count — that a fleet publisher writes with `workspace.report_metadata`, the same path `lifecycle`, `outcome` and `quota_5h` already take. A project at 5,000 files or more draws at its tier's largest, half the sun's radius, so no planet ever rivals the star it orbits; below that the radius follows the cube root of the count, because volume tracks mass and that is what made size legible as size. **An absent or unreadable token is not a size of zero**: a project nobody has measured draws at the register's floor — a real body a little over 40% of the largest, never a dot — so wiring up no publisher at all leaves a scene that reads exactly as it did before. Two bodies stay out of the register entirely: the sun, which routes to projects rather than being one of them, and every worker moon, which is a pane rather than a checkout.

`on` sets both `[experimental] persistent_background` and `[experimental] kitty_graphics` in `config.toml`, because the scene is a Kitty Graphics surface and needs both; setting only the first is a silent no-op. `off` clears `persistent_background` and deliberately leaves `kitty_graphics` alone, since the sidebar's pixel cards and tray art read it too. Neither reaches into a running server: run `herdr server reload-config` afterwards to apply the change.

`status` prints every condition the scene is gated on, met or not:

```text
background scene: DRAWING

  yes [experimental] persistent_background
  yes [experimental] kitty_graphics
  yes host terminal draws an ambient wash under the text (identified as: kitty)
  yes every attached viewer draws one too
  yes host answered the Kitty Graphics probe (not a gate; other pixel surfaces need it)
```

The terminal condition is the one config alone cannot answer. The scene is an opaque image placed *under* the text, and a terminal that accepts the placement but ignores the negative-z band draws that same image over every glyph instead — so Herdr refuses it on any terminal it has not positively identified. The identity is read in band over the pty (XTVERSION), which means it survives an SSH hop where `TERM_PROGRAM` does not: `other` means the terminal did not answer or gave a name Herdr does not know, not that a variable was unset.

The same conditions are on the JSON API as `background_scene` in the session snapshot.
...
```bash
herdr api status get
herdr api status set <text>
herdr api status clear
```

The status is one line of your own text about the session. Herdr never composes or interprets it — whatever you publish is what the sidebar shows, on the header row above the first Space in its tree. A script on a timer can put a quota readout, a deploy banner, or a build number there without Herdr learning about any of them:

```bash
herdr api status set "7d 62% · 5h 18%"
```

Nothing is drawn until something sets a status, `clear` returns the row to empty, and a long status elides on a narrow sidebar rather than pushing the tree around.

It sits under `herdr api` rather than `herdr session` for the same reason `herdr api snapshot` does: `herdr session` is the session manager, where every subcommand names a session to act on, while this acts on the session the global `--session` flag already selected.
...
```bash
herdr pane read <pane_id> [--source visible|recent|recent-unwrapped|detection|transcript] [--lines N] [--format text|ansi] [--ansi] [--raw]
...
Declare which agent a pane hosts when Herdr's process probe cannot see it — for example an agent started behind a wrapper script or inside a terminal multiplexer:

```bash
herdr pane declare-agent <pane_id> --agent LABEL
herdr pane declare-agent <pane_id> --clear
```

Unlike `report-agent`, this reports no state. It supplies identity only, and Herdr's normal screen detection then runs for that agent and produces live state. Process identity stays authoritative: the declaration is used only while the process probe finds no agent, so a directly launched agent is unaffected. The declaration is durable — it survives a server restart and a live handoff until it is cleared. `pane get` and `pane list` report it as `declared_agent`.

When neither the process probe nor a declaration knows the agent, Herdr falls back to recognizing the agent from the pane's own screen using `[[identity]]` rules in the bundled manifests. Only agents whose manifest ships identity rules can be identified this way, an ambiguous screen yields nothing rather than a guess, and the answer must survive several consecutive scans before it is adopted. Check the rules against a captured screen with:

```bash
herdr agent identify --file PATH [--osc-title TEXT] [--osc-progress TEXT] [--json]
```

Capture the screen with `herdr agent read <pane> --source detection --format text` (or `herdr pane read`), and read the OSC title out of `herdr pane get <pane>`'s `terminal_title`.
...
Report the running edits the pane's agent has made this session. This is what the Changes zone draws for the focused pane — that zone reads this per-pane edit log, not the pane's `git diff`:

```bash
herdr pane report-edit-diff <pane_id> \
  --file PATH \
  --diff - \
  [--clear-all]
```

`--diff` accepts only `-`, and reads the diff from stdin. A unified diff is newline-heavy, arbitrarily long text, which is the worst thing to carry in an argv value, and the callers are hook scripts that already have it on a pipe:

```bash
diff -u "$BASELINE" "$FILE" | herdr pane report-edit-diff "$PANE_ID" --file "$FILE" --diff -
```

Each call replaces that file's previous entry; it never appends. Snapshot each file's content once, the first time the agent touches it in a session, and recompute the whole diff against that same fixed baseline on every later edit, so three edits to one file stay one clean diff instead of three stacked hunks. `--file` is the path as you want it displayed; it is not resolved or checked against the pane's cwd, but it is trimmed, stripped of control characters, and capped at 4096 characters. The diff does not need a `diff --git` line — Herdr synthesizes one from `--file` so each reported file opens under its own header card.

Omitting `--diff`, or piping in nothing, clears that one file's entry — send that when an edit is reverted back to its baseline. `--clear-all` ignores `--file` and empties the pane's whole log instead; send it when a new agent session takes over the pane, so it does not inherit the previous session's edits. `--file` is required for every call that is not `--clear-all`.

One call's diff is capped at 4000 lines, counting the `diff --git` line Herdr synthesizes — that bounds one file's report, not the pane's whole log, which has no total ceiling. A longer one is rejected whole rather than stored truncated. The log is display-only, takes no lifecycle authority, and is not restored after a server restart.

Herdr does not capture these edits itself: this is the door for a `PreToolUse`/`PostToolUse` hook pair installed beside the managed integration hook. The bundled Claude Code integration sends only `--clear-all`, on `SessionStart`.
...
herdr agent read <target> [--source visible|recent|recent-unwrapped|detection|transcript] [--lines N] [--format text|ansi] [--ansi]
...
```bash
herdr pane wait-output <pane_id> (--match <text> | --regex <pattern>) [--source visible|recent|recent-unwrapped|transcript] [--lines N] [--timeout MS] [--raw]
```
...
| `transcript` | Recent scrollback with the agent's composer excluded, so a reader cannot mistake unsent input for agent output. |
...
`transcript` reads the same rows as `recent` and then removes the agent's
composer/prompt box, using the `transcript_region` declared by that agent's
detection manifest. It is the source to use when another program reads a pane
and must not treat text sitting unsent in the input line as something the agent
said. Panes with no detected agent, or whose agent manifest declares no
`transcript_region`, return the unmodified `recent` bytes; the JSON read result
reports which happened in `transcript_applied`. `transcript` changes only what
is excluded, never how far back a read reaches — alternate-screen output that
never entered the host scrollback is unreachable from every source.
