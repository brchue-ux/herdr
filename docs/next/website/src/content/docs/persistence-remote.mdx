...
Several clients can be attached to one session at the same time, and they share the panes rather than each getting their own layout. The panes are sized for whichever client was active most recently, so if another client of a different terminal size attaches and takes over, your panes keep the other client's size: content fills only part of each pane, or runs off the bottom. Herdr says so — a client in that state shows `panes pinned to <cols>x<rows> by another client` at the top right, naming the size that currently owns the panes. It clears by itself once the other client detaches or both terminals are the same size.
...
In this mode your local Herdr is a thin client. It connects over SSH, starts or attaches to the remote Herdr server, and streams the UI back to your local terminal. Because the client runs locally, Herdr can bridge local desktop features into the remote session: image clipboard paste is bridged by copying the image to a remote temp file and pasting that path, and pasting into a Herdr prompt — renaming a workspace, tab, or pane, naming a new linked worktree, or any of the search boxes — reads your local clipboard rather than the server's, even when your terminal delivers the paste as a plain `Ctrl+V` key instead of a bracketed paste.
...
Remote attach supports Linux and macOS hosts on x86_64 and aarch64 — this is about the remote server's platform, not the machine you attach from. `herdr --remote` also runs from the Windows binary. Herdr checks the remote platform, prefers a matching `herdr` already on the remote `PATH`, then checks common direct, Homebrew, mise, and Nix profile install paths. If no matching binary exists, interactive runs prompt to install one to `~/.local/bin/herdr`; non-interactive runs fail instead of modifying the host. If `~/.local/bin` is not on the remote `PATH`, Herdr warns after install.

`herdr --remote` runs remote setup and the bridge through a temporary SSH config that includes your own SSH config first, then adds fallback keepalive settings. Existing user keepalive settings win. Without that fallback OpenSSH defaults to `ServerAliveInterval 0`, so a connection that dies silently — a sleeping laptop, dropped wifi, a VPN cutting out — is not noticed until TCP itself gives up. On macOS and Linux the generated config also sets up a private per-attach control socket for connection reuse; Windows OpenSSH has no ControlMaster support, so on Windows the generated config carries the keepalive fallback only and every ssh invocation runs unmultiplexed. Set `[remote].manage_ssh_config = false` to use plain `ssh` without Herdr's generated config or control socket.
