...
| Session | `session.snapshot`, `session.status.set`, `session.status.clear` |
...
| Pane | `pane.split`, `pane.swap`, `pane.move`, `pane.zoom`, `pane.minimize`, `pane.dormant.reappear`, `pane.layout`, `pane.process_info`, `pane.neighbor`, `pane.edges`, `pane.focus_direction`, `pane.resize`, `pane.list`, `pane.current`, `pane.get`, `pane.rename`, `pane.send_text`, `pane.send_keys`, `pane.send_input`, `pane.read`, `pane.graphics.info`, `pane.graphics.set`, `pane.graphics.clear`, `pane.graphics.stream`, `pane.report_agent`, `pane.report_agent_session`, `pane.report_metadata`, `pane.report_edit_diff`, `pane.clear_agent_authority`, `pane.release_agent`, `pane.declare_agent`, `pane.close`, `pane.wait_for_output` |
| Surface | `surface.graphics.info`, `surface.graphics.set`, `surface.graphics.clear` |
| Popup | `popup.close` |
| Layout | `layout.export`, `layout.apply`, `layout.set_split_ratio`, `layout.arrange` |
...
tab layout snapshots, agent records, an `agent_view` object naming the
active Agents view when one is in force, and the `status` string when one is
set. It is not a subscription; after reading
...
`session.status.set` publishes one line of caller-supplied text about the
session, and `session.status.clear` removes it. Herdr never composes or
interprets the text: the slot is content-agnostic, so a quota readout, a deploy
banner, and a build number all reach it the same way. The desktop TUI renders it
on the sidebar's reserved header row, above the first Space in the tree; other
clients are free to place it elsewhere or ignore it.

```json
{"id": "1", "method": "session.status.set", "params": {"status": "7d 62% · 5h 18%"}}
```

Both methods answer with `{"type": "session_status", "status": ...}` carrying
whatever is left in force, and `status` is absent when nothing is set. Control
characters are stripped and the text is capped, so a status that sanitizes down
to nothing clears the slot instead of setting it to an empty string. The status
is deliberately not persisted across a server restart: it is a claim about the
world right now, and its publisher republishes on its own clock.

From the CLI this is `herdr api status get|set|clear`.
...
`PaneInfo` includes `activity` once the pane has been sampled at least once:
...
`level_percent` is how hard the pane is working right now, on a `0..=100`
scale. Herdr samples every pane's terminal output on its own loop and smooths
the result, so the value rises quickly when output starts and eases back to `0`
over a few seconds after it stops. It is unitless on purpose: the curve from
output rate onto this scale is a Herdr-side presentation decision and can be
re-tuned between releases, so bind to `level_percent` rather than deriving your
own scale from the rate.

`bytes_per_sec` is the raw output rate measured over the most recent sample
window, unsmoothed, and `output_bytes` is a lifetime counter for the pane's
terminal. Both are there for tuning and diagnosis. `output_bytes` restarts when
a new process takes over the terminal, so treat a decrease as a restart rather
than as negative work.

This is reported rather than pushed: there is no activity event, because a
continuously varying value on a fixed sample cadence would turn into an event
stream that scales with how loud a pane is. Poll `pane.get` or `pane.list` at
whatever rate you need. It is also not persisted — a freshly started server
reports every pane at rest until its next few samples land, and a pane that has
never produced output has no `activity` object at all.
...
`placement.z` picks which band the image is drawn in, using Kitty's z-index
rules. `0` — the default, and the only value earlier releases used — draws over
the cell's text. Negative values draw under the text but over the cell
background. Values below `-1073741824` draw under the background too, which is
the band a backdrop needs so it does not erase what sits on top of it.

Regions that are not panes have no pane id, so they are addressed by name
through `surface.graphics.set`, `surface.graphics.clear`, and
`surface.graphics.info` instead. `surface` currently accepts `sidebar`. These
take the same formats, limits, placement object, and `feature_disabled` gate as
the pane methods, and the image is clipped to the named region: nothing placed
on the sidebar can spill into a pane, and a layout without a sidebar — the
mobile layout, or a fully hidden sidebar — draws nothing while keeping the
layer, so it reappears when the region comes back.

```json
{"id":"surface_info","method":"surface.graphics.info","params":{"surface":"sidebar"}}
{"id":"surface_set","method":"surface.graphics.set","params":{"surface":"sidebar","format":"png","image_width":260,"image_height":800,"data_base64":"...","placement":{"viewport_col":0,"viewport_row":0,"grid_cols":26,"grid_rows":40,"z":-1073741825}}}
{"id":"surface_clear","method":"surface.graphics.clear","params":{"surface":"sidebar"}}
```
...
`layout.arrange` rebuilds a tab's existing panes into a built-in arrangement.
Unlike `layout.apply` it never creates, closes, or relaunches a pane: pane ids,
their order, and the focused pane all survive, and only the splits and ratios
change. Zoom is cleared so the new arrangement is visible. `arrangement` accepts
`even_horizontal`, `even_vertical`, `main_vertical`, `main_horizontal`, `tiled`,
or the relative steps `next` and `previous`, which walk that list in order from
the last arrangement applied to the tab. The response is `type:
"layout_arrange"` with the concrete `arrangement` that was applied and the
updated portable `layout`.

```json
{"id":"req_arrange","method":"layout.arrange","params":{"tab_id":"w1:t1","arrangement":"next"}}
```
...
`pane.minimize` detaches the whole tab `pane_id` belongs to from its
workspace's live tree — the tab and every pane inside it, together, as one
unit — without touching any of their `TerminalRuntime`s. A minimized tab's
processes keep running and its panes keep reporting live agent status; it is
simply not attached anywhere a client can see it:

```json
{"id":"req_minimize","method":"pane.minimize","params":{"pane_id":"w1:p2"}}
```

Omitting `pane_id` targets the server's active focused pane. The response is
`type: "pane_minimize"` with `changed`, optional `reason`, `terminal_ids` (the
terminal id of every pane that was in the minimized tab), and `workspace_id`.
The only reason value is `only_tab_in_workspace` — minimizing a workspace's
last tab is refused rather than leaving the workspace with no visible tab.
Herdr does not enforce which tabs may be minimized beyond that; a caller is
expected not to minimize a tab it wants to keep visible.

`pane.dormant.reappear` reattaches a minimized tab, keyed by `terminal_id` —
once a tab is minimized its `pane_id` no longer resolves, so `terminal_id` is
the only handle that survives:

```json
{"id":"req_reappear","method":"pane.dormant.reappear","params":{"terminal_id":"term_abc123","focus":false}}
```

`focus` defaults to `false` so a reappear does not steal attention by default.
The response is `type: "pane_dormant_reappear"` with `changed`, optional
`reason`, and the reattached `pane` — this is the only place a caller learns
the pane's freshly assigned public `pane_id`, since minimizing frees its old
number for reuse. The call is idempotent: reappearing an already-visible pane
(a retried call, or one reattached through some other path) returns
`changed: false` with `reason: "already_visible"` and the pane's current
location, rather than erroring or moving it again. A `terminal_id` that was
never minimized, was already explicitly closed while dormant, or belonged to a
process that has since exited returns `code: "not_found"`.

A minimized tab reappearing — whether through an explicit
`pane.dormant.reappear` call or automatically (see below) — emits the same
`pane.created` and `layout.updated` events a genuine new pane spawn would.
Nothing in the event stream lets a subscriber tell a reappear apart from a
fresh spawn, matching how `pane.move` already keeps a moved pane's terminal
identity opaque to subscribers.

Herdr also reappears a dormant pane automatically, with no caller involved,
the instant that pane's live agent status transitions into `working` — the
same signal a plain caller watches to learn work has resumed. A pane dormant
while already `working` is unaffected (nothing "started"), and a transition
into `idle`, `blocked`, or `unknown` does not trigger it (work finishing or
staying blocked is not work starting). This auto-reappear is not configurable.
...
Use at most one of `workspace_id` or `cwd` for `worktree.list`, `worktree.create`, and `worktree.open`; omit both to use the active workspace. Use exactly one of `path` or `branch` for `worktree.open`. Raw socket `cwd` and `path` values must be absolute; the CLI expands relative `--cwd` and `--path` values before sending requests. Workspace responses include optional `worktree` provenance whenever the workspace sits in a Git checkout, whether Herdr created that checkout through a worktree command or the workspace was simply started there; `repo_key` is the same value for every workspace in one repository. Workspaces outside a Git work tree omit the field. Worktree commands can emit `workspace.updated` when an existing workspace gains or changes worktree provenance.
...
may use their own non-`plugin:` source. `config` and `ui` are reserved for
Herdr's own tiers and are rejected with `invalid_agent_view`. A successful set
atomically replaces the previous API-owned view. The view lasts until it is
cleared, replaced, its owning plugin is disabled, unlinked, or uninstalled, or
the server exits. Plugins that want durable behavior should save the query under
`HERDR_PLUGIN_STATE_DIR` and reapply it from a `[[startup]]` hook.

Three tiers can own the Agents view, ranked `config` < `ui` < `plugin`. The
socket API owns the `plugin` tier only, and it is the highest, so
`agent.view.set` always takes the panel. The `config` tier is the declarative
[`[ui.sidebar.agents.view]`](/docs/configuration/#agents-panel-view) config key,
which is durable across restarts and is never overwritten or cleared by an API
call. The `ui` tier is reserved for views Herdr sets for itself.
...
`type: "agent_view"` and report `active`, `source`, and optional `label`. Because
clearing the API tier reveals a lower tier rather than an unfiltered panel, a
clear response can report `active: true` with `source: "config"`.
...
## Declaring which agent a pane hosts

Herdr identifies a pane's agent from its process tree. A wrapper script, a
launcher, or a terminal multiplexer can hide the agent process, leaving the pane
`unknown` even though an agent is plainly running in it.

Two things close that gap, in this order:

1. **The declaration.** `pane.declare_agent` states the answer outright. It
   reports no state — only identity — after which Herdr's normal screen
   detection runs for that agent and produces live state. Send `agent: null` to
   clear it.

   ```json
   {
     "id": "req_1",
     "method": "pane.declare_agent",
     "params": { "pane_id": "w1:p1", "agent": "claude" }
   }
   ```

   Process identity stays authoritative: the declaration is consulted only while
   the process probe finds no agent, so a directly launched agent is unaffected.
   A pending `pane.release_agent` outranks it. The declaration is durable — it
   survives a server restart and a live handoff until cleared — and `pane.get`
   and `pane.list` report it as `declared_agent`.

2. **Screen identification.** When neither the process probe nor a declaration
   knows the agent, Herdr tries to recognize the agent from the pane's own
   screen, using `[[identity]]` rules in the bundled agent manifests. This needs
   no cooperation from the agent and no configuration.

   It is deliberately conservative, because a misidentified pane reports another
   agent's state as fact: only agents whose manifest ships identity rules can be
   identified at all, two agents matching the same screen is treated as
   ambiguous and yields nothing, and the same answer has to survive several
   consecutive scans before it is adopted. An identification is discarded as
   soon as the pane's foreground process group changes.

   `herdr agent identify --file PATH [--osc-title TEXT]` runs the same rules
   against a captured screen, which is how identity rules are written and
   checked.
...
`agent.list` and `agent.get` also expose `relation` (`first_mate`, `second_mate`, or `worker`) and `owner` — the resolved place a pane's `owner` metadata token gives it in the fleet's ownership tree, the same tag-and-group projection the sidebar and the persistent background scene already draw. Both are omitted for a pane the tree draws no row for on its own, such as a mate's own driving pane, whose place is carried by its Space instead.

`pane.get`, `pane.list`, `agent.get`, `agent.list`, `workspace.get`, and `workspace.list` expose `absorbed`: how many finished workers this mate has taken back. It is counted from the `completed` reports a fleet already sends with `workspace.report_signal`, credited to the `owner` the ownership tree resolves for the row that finished — so a worker's completion increments the mate it is drawn under, and a second mate's own completion increments the first mate above it. The count is idempotent under `seq`, so a retried report does not double it. It is omitted when zero, is not persisted across a cold restart, and is uncapped here even though the sidebar draws at most eight rings for it.
...
Use `pane.report_edit_diff` when a hook can report the files the pane's agent has edited this session. It is what the Changes zone draws for the focused pane: that zone reads this per-pane edit log, not the pane's `git diff`.
...
```json
{
  ...
  "params": {
    ...
    "diff": "--- a/src/auth/middleware.rs\n+++ b/src/auth/middleware.rs\n@@ -12,3 +12,4 @@\n fn authorize() {\n-    legacy_check();\n+    verify_token()?;\n+    audit(\"authorize\");\n }\n"
  }
}
```

Each report replaces that file's previous entry rather than appending to it. A reporter is expected to snapshot each file's content once, the first time the agent touches it in a session, and to recompute the whole diff against that same fixed baseline on every later edit, so three edits to one file stay one clean diff instead of three stacked hunks. Diffing against the last report instead produces a log the zone cannot read as cumulative change.

`diff` is unified-diff text, such as `diff -u` output. It does not have to carry a `diff --git a/x b/x` line: Herdr synthesizes one from `file`, so every reported file opens under its own header card without the reporter having to know that convention. `file` is the path as you want it displayed. It is not resolved, validated, or checked against the pane's cwd; it is trimmed, stripped of control characters, and capped at 4096 characters, so that it stays the single line the synthesized header needs.

An omitted or empty `diff` clears that one file's entry — the report to send when an edit is reverted back to its baseline. `clear_all` ignores `file` and `diff` and empties the pane's whole log instead; send it when a new agent session takes over the pane, so the new session does not inherit the previous one's edits. `file` may be empty only in that case, and is otherwise rejected with `invalid_edit_diff_file`.

Each report is capped at 4000 lines, counting the `diff --git` line Herdr synthesizes: one file's diff, not the pane's whole log, which has no total ceiling. A longer report is rejected whole with `invalid_edit_diff_too_large` rather than stored truncated, so a reporter is never told a diff was kept in full when it was not. An unknown `pane_id` returns `pane_not_found`.

The log lives on the pane's terminal, is display-only, takes no lifecycle authority, emits no event, and is not restored after a server restart. Herdr does not capture these edits itself: `herdr pane report-edit-diff` is the CLI door for a `PreToolUse`/`PostToolUse` hook pair installed beside the managed integration hook. The bundled Claude Code integration sends only the `clear_all` report, on `SessionStart`.
...
`pane.moved`, `pane.exited`, `pane.dormant.exited`, `pane.agent_detected`,
...
`pane.dormant.exited` reports a minimized pane's process exiting — it carries
`terminal_id` instead of `pane_id`/`workspace_id`, since a dormant pane has
neither; `pane.exited` never fires for a dormant pane, because it requires a
live `pane_id` to report under.
...
herdr pane read w1:p1 --source transcript --lines 120
```
...
`transcript` returns the `recent` rows with the agent's composer/prompt box cut
off, using the `transcript_region` declared by that agent's detection manifest,
so a program reading the pane cannot mistake unsent input for agent output. When
the pane has no detected agent, or its manifest declares no `transcript_region`,
the read falls back to the unmodified `recent` bytes. `.result.read.transcript_applied`
is `true` when the composer was actually cut and `false` when the read fell back;
it is absent for every other source.
