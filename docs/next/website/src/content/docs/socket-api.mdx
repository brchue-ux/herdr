...
Workspace ids such as `w1` are stable identity and are independent of display
order. They are resolved by identity only: a `workspace_id` that no longer names
an open workspace returns `workspace_not_found` rather than resolving to the
workspace currently in that position. Long-running publishers that cache
`HERDR_WORKSPACE_ID` should treat `workspace_not_found` as the signal that the
cached id is stale and re-read it, rather than assuming a successful write
reached the intended Space.
