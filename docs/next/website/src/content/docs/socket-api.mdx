...
`diff` is unified-diff text, such as `diff -u` output. It does not have to carry a `diff --git a/x b/x` line: Herdr synthesizes one from `file`, so every reported file opens under its own header card without the reporter having to know that convention. `file` is the path as you want it displayed. It is not resolved, validated, or checked against the pane's cwd.
...
One report is capped at 4000 lines — the same ceiling the zone renders to — and a longer one is rejected whole with `invalid_edit_diff_too_large` rather than stored truncated, so a reporter is never told a diff was kept in full when it was not. An unknown `pane_id` returns `pane_not_found`.
