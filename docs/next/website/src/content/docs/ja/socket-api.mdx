...
| ポップアップ | `popup.close` |
| レイアウト | `layout.export`、`layout.apply`、`layout.set_split_ratio`、`layout.arrange` |
...
`session.snapshot` は、独自のローカルランタイムキャッシュを持つクライアント向けに、一度限りのブートストラップスナップショットを返します。レスポンスには、バージョン/プロトコルメタデータ、フォーカス中のワークスペース/タブ/ペイン id、ワークスペース、タブ、ペイン、タブレイアウト、エージェントの各レコードと、Agents ビューが有効な場合はそれを示す `agent_view` オブジェクトが含まれます。これは購読ではありません。取得後はリソースイベントを購読し、そのイベントでローカルキャッシュを更新してください。再接続後やキャッシュが古い可能性がある場合は、`session.snapshot` を再度呼び出します。ワークスペースレコードには、関連付けられた worktree の出自情報も含まれます。リポジトリ全体の worktree 検出には引き続き `worktree.list` を使います。
...
ペインが一度でもサンプリングされると、`PaneInfo` に `activity` が含まれます:
...
`level_percent` は、そのペインが今どれだけ働いているかを `0..=100` の尺度で表します。Herdr は自身のループで各ペインのターミナル出力をサンプリングして平滑化するため、出力が始まると値は素早く上がり、止まると数秒かけて `0` に戻ります。これは意図的に無単位です。出力レートからこの尺度への変換は Herdr 側の表示上の判断であり、リリース間で調整され得ます。レートから独自の尺度を導出せず、`level_percent` を使ってください。

`bytes_per_sec` は直近のサンプル窓で測定した生の出力レート(平滑化なし)、`output_bytes` はそのペインのターミナルの累積カウンタです。どちらも調整と診断のためのものです。`output_bytes` はターミナルを新しいプロセスが引き継ぐとリセットされるため、値の減少は負の作業量ではなく再起動として扱ってください。

これはプッシュではなくポーリングで取得します。activity イベントは存在しません。固定のサンプル周期で連続的に変化する値をイベント化すると、ペインの騒がしさに比例したイベントストリームになってしまうためです。必要な間隔で `pane.get` または `pane.list` を呼んでください。永続化もされません。起動直後のサーバーは次の数サンプルが届くまで全ペインを静止状態として報告し、出力を一度も出していないペインには `activity` オブジェクト自体がありません。
...
独自の非 `plugin:` source を使えます。`config` と `ui` は Herdr 自身の層のために予約
されており、`invalid_agent_view` で拒否されます。設定に成功すると以前の API 所有ビューは
アトミックに置き換えられます。ビューは clear または置換されるか、所有プラグインが
disable、unlink、uninstall されるか、サーバーが終了するまで有効です。永続的な動作が必要な
プラグインは、クエリを `HERDR_PLUGIN_STATE_DIR` に保存し、`[[startup]]` フックから
再適用してください。

Agents ビューを所有できる層は 3 つあり、優先順位は `config` < `ui` < `plugin` です。
ソケット API が所有するのは最上位の `plugin` 層だけなので、`agent.view.set` は常に
パネルを取得します。`config` 層は宣言的な
[`[ui.sidebar.agents.view]`](/docs/configuration/#エージェントパネルビュー) 設定キーで、
再起動をまたいで永続し、API 呼び出しから上書きも clear もされません。`ui` 層は Herdr が
自身のために設定するビュー用に予約されています。
