...
有効な場合、`herdr --remote` は最初にあなた自身の SSH 設定ファイルを取り込み、その後にフォールバックの `ServerAliveInterval` と `ServerAliveCountMax` の値を加えた、プライベートな一時 SSH 設定を書き込みます。ユーザー自身のキープアライブ設定が優先され、無効化するための明示的な `ServerAliveInterval 0` もそのまま尊重されます。macOS と Linux で取り込まれるのは `~/.ssh/config` と `/etc/ssh/ssh_config` で、Herdr は最初に認証した接続を再利用するため、アタッチごとにプライベートな OpenSSH コントロールソケットも使います。Windows で取り込まれるのは `%USERPROFILE%\.ssh\config` と `%PROGRAMDATA%\ssh\ssh_config` で、Windows の OpenSSH には ControlMaster のサポートがないためコントロールソケットは使われず、生成される設定はキープアライブのフォールバックだけを含みます。どのプラットフォームでも、Herdr が生成した設定やコントロールソケットを使わず通常の `ssh` でリモートアタッチを実行するには `manage_ssh_config = false` を設定してください。
...
- `terminal_title` — 1 つのペインから集約した、安全性のための正規化後の最新の OSC 0/2 ターミナルタイトル。
- `terminal_title_stripped` — 同じく集約したタイトルから、認識された先頭のアクティビティまたはスピナーのグリフ 1 つと、それに続く空白を除去したもの。
- `$name` — `name` という名前のカスタムワークスペースメタデータ。

1 つの Space には複数のペインが含まれるため、ターミナルタイトルは `state_icon` と `state_text` が示すペインと同じペインから集約します。対象は注目度が最も高いペイン (blocked、次に未確認の完了、working、idle、unknown の順) で、同順位の場合はタブとレイアウト順で最初のペインです。したがって、1 つの Space 行に並ぶアイコンとタイトルは常に同じペインを表します。ペインがない Space、実行中のターミナルがない Space、選ばれたペインがターミナルタイトルを設定していない Space では、タイトルは表示されません。ほかの値がない場合と同様に区切りとともに消え、その行の他のトークンにも値がなければ行ごと表示されません。折りたたまれた worktree グループの状態アイコンはメンバー Space 全体で集約されますが、各行のタイトルは `workspace` や `branch` と同じくその Space 自身のものです。
...
`[ui.sidebar.agents.view]` は、Agents パネルに表示するエージェントとその並び順を宣言します。フィルタとソートの文法は [`agent.view.set`](/docs/socket-api/#agent-view) ソケットコマンドと同一です。`agent.view.set` と違い、宣言されたビューは起動時と設定リロードごとに `config.toml` から読み込まれるため、外部プロセスが再適用しなくてもサーバー再起動をまたいで残ります。
...
`filter` は `status`、`workspace_id`、`tab_id`、`pane_id`、`agent`、`seen`、`state_change_seq`、および `{ token = "name" }` のペインメタデータに対する `all`、`any`、`not`、`eq`、`in`、`exists` の木構造です。`sort` は `workspace_order`、`tab_order`、`pane_order`、`attention`、`status`、`agent`、`seen`、`state_change_seq`、`{ token = "name" }` を対象とする `{ field, order }` テーブルの順序付きリストです。`order` は `"asc"` または `"desc"` で、既定は `"asc"` です。`sort` が空でない場合、ビューが有効な間は `ui.agent_panel_sort` を置き換えます。ネスト 8 段、フィルタノード 64 個、`in` の値 32 個、ソートキー 8 個という上限はソケットコマンドと同じです。

Agents ビューを所有できるソースは 3 つあり、優先順位は `config` < `ui` < `plugin` です:

- **`config`** — このキー。ユーザーが一度書き留めた標準の既定値です。
- **`ui`** — Herdr 自身がセッション中に設定するビュー。その場の操作は保存された設定より具体的なので、標準の既定値を一時的に上書きします。
- **`plugin`** — `agent.view.set` で設定されたビュー。意図してインストールし有効化したプラグインの指示なので、設定ファイルの 1 行がそれを黙って無効化しないよう最優先になります。

`agent.view.set` と `agent.view.clear` が所有するのは plugin 層だけです。`config` と `ui` は予約されたソース名で、ソケット API からは拒否されます。プラグインのビューをクリアすると、フィルタなしのパネルに飛ぶのではなく config のビューが再び現れます。

パネルのヘッダーは常に担当している層を表示するので、フィルタが見えなくなることはありません:

```text
 agents      config:needs me · 3 hidden !
```

数値はビューが隠しているエージェント数で、末尾の `!` は隠れているエージェントのうち 1 つが blocked で入力を待っていることを意味します。サイドバーが狭い場合は層の接頭辞、次に `hidden` の語が省略されますが、件数は決して省略されません。ヘッダーをクリックするとビューを所有する層がクリアされ、1 つ下の層が現れます。config で宣言したビューは次の設定リロードまたは再起動で戻ります。

不正なビューは適用されません。Herdr はそれを無視してすべてのエージェントを表示し、設定診断バナーと `herdr config check` で問題を報告します:
