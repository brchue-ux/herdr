...
## Reading agent output without reading its composer

`--source transcript` reads the same rows as `recent` and then removes the agent's composer/prompt box. Use it whenever another program reads a pane and must not treat text sitting unsent in the input line — a pre-filled suggestion, a queued prompt — as something the agent said. It does not reach further back than `recent`, so it is no help with the alternate-screen caveat above; it only removes a source of misreads. Panes with no detected agent, or agents whose detection manifest declares no `transcript_region`, return the unmodified `recent` bytes and report `.result.read.transcript_applied` as `false`.

```bash
herdr agent read reviewer --source transcript --lines 120
```
