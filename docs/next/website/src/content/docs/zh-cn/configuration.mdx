...
启用后，`herdr --remote` 会写入一份私有的临时 SSH 配置：先包含你自己的 SSH 配置文件，再添加兜底的 `ServerAliveInterval` 和 `ServerAliveCountMax` 值。你自己的保活设置优先，包括用显式的 `ServerAliveInterval 0` 关闭保活。在 macOS 和 Linux 上包含的是 `~/.ssh/config` 和 `/etc/ssh/ssh_config`，Herdr 还会为每次远程连接使用私有的 OpenSSH control socket，以复用首次通过身份验证的连接。在 Windows 上包含的是 `%USERPROFILE%\.ssh\config` 和 `%PROGRAMDATA%\ssh\ssh_config`，并且不使用 control socket，因为 Windows OpenSSH 不支持 ControlMaster，生成的配置只带保活兜底。在任何平台上，设置 `manage_ssh_config = false` 都可通过普通 `ssh` 进行远程连接，不使用 Herdr 生成的配置或 control socket。
...
- `terminal_title` — 从单个窗格汇总的、经过安全规范化的最新 OSC 0/2 终端标题。
- `terminal_title_stripped` — 对同一汇总标题，移除一个可识别的开头活动或旋转指示符字形及其后空白后的结果。
- `$name` — 名为 `name` 的自定义工作区元数据。

一个 Space 可以包含多个窗格，因此它的终端标题取自与 `state_icon`、`state_text` 相同的窗格：注意力优先级最高的窗格（依次为 blocked、未查看的已完成、working、idle、unknown），优先级相同时取标签页与布局顺序中的第一个窗格。因此同一 Space 行上的图标与标题始终描述同一个窗格。没有窗格、没有运行中的终端，或所选窗格从未设置终端标题的 Space 不会渲染标题；与其他缺失值一样，它会连同分隔符一起消失，当该行其余 token 也没有值时整行消失。折叠的 worktree 分组会跨成员 Space 汇总状态图标，但每行的标题仍是该 Space 自身的标题，这与 `workspace` 和 `branch` 一致。
...
`[ui.sidebar.agents.view]` 声明 Agents 面板显示哪些智能体以及排列顺序，使用与 [`agent.view.set`](/docs/socket-api/#agent-view) 套接字命令完全相同的过滤和排序语法。与 `agent.view.set` 不同，声明式视图在启动时以及每次配置重载时从 `config.toml` 读取，因此无需外部进程重新应用即可跨服务器重启保留。
...
`filter` 是由 `all`、`any`、`not`、`eq`、`in`、`exists` 组成的树，作用于 `status`、`workspace_id`、`tab_id`、`pane_id`、`agent`、`seen`、`state_change_seq` 以及 `{ token = "name" }` 窗格元数据。`sort` 是 `{ field, order }` 表的有序列表，字段可为 `workspace_order`、`tab_order`、`pane_order`、`attention`、`status`、`agent`、`seen`、`state_change_seq` 或 `{ token = "name" }`。`order` 为 `"asc"` 或 `"desc"`，默认 `"asc"`。非空的 `sort` 会在视图生效期间替代 `ui.agent_panel_sort`。限制与套接字命令相同：嵌套 8 层、过滤节点 64 个、每个 `in` 最多 32 个值、最多 8 个排序键。

有三个来源可以拥有 Agents 视图，优先级为 `config` < `ui` < `plugin`：

- **`config`** — 本配置键。用户一次写下的长期默认值。
- **`ui`** — Herdr 自身为当前会话设置的视图。即时操作比存储的偏好更具体，因此临时覆盖长期默认值。
- **`plugin`** — 通过 `agent.view.set` 设置的视图。它优先级最高：插件是用户有意安装并启用的，一行配置不应静默地让它失效。

`agent.view.set` 和 `agent.view.clear` 只拥有 plugin 层。`config` 和 `ui` 是保留的来源名称，套接字 API 会拒绝它们；清除插件视图会重新显示 config 视图，而不是跳到未过滤的面板。

面板标题始终标明当前负责的层，因此过滤状态永远不会隐形：

```text
 agents      config:needs me · 3 hidden !
```

数字表示视图隐藏了多少个智能体，结尾的 `!` 表示其中有一个处于 blocked 状态并在等待你。侧边栏较窄时会先去掉层前缀，再去掉 `hidden` 一词，但绝不会去掉数量。点击标题会清除拥有该视图的层，露出下一层；config 声明的视图会在下次配置重载或重启后回来。

无效的视图永远不会被应用。Herdr 会忽略它、显示全部智能体，并通过配置诊断横幅和 `herdr config check` 报告问题：
