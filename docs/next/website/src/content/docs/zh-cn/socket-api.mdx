...
| 弹窗 | `popup.close` |
| 布局 | `layout.export`、`layout.apply`、`layout.set_split_ratio`、`layout.arrange` |
...
`session.snapshot` 为维护本地运行时缓存的客户端返回一次性引导快照。响应包含版本/协议元数据、当前聚焦的工作区/标签页/窗格 id、工作区记录、标签页记录、窗格记录、标签页布局快照、智能体记录,以及在 Agents 视图生效时用于说明该视图的 `agent_view` 对象。它不是订阅;读取后应订阅资源事件,并根据事件更新本地缓存。重新连接后或本地缓存可能已过期时,再次调用 `session.snapshot`。工作区记录也包含关联 worktree 的来源信息。完整的仓库 worktree 发现仍使用 `worktree.list`。
...
窗格至少被采样一次后,`PaneInfo` 包含 `activity`:
...
`level_percent` 表示窗格当前的工作强度,取值范围 `0..=100`。Herdr 在自己的循环中采样每个窗格的终端输出并做平滑处理,因此输出开始时该值迅速上升,输出停止后在几秒内回落到 `0`。它刻意不带单位:从输出速率映射到该尺度是 Herdr 侧的呈现决策,可能在版本之间调整,所以请绑定 `level_percent`,不要根据速率自行推导尺度。

`bytes_per_sec` 是最近一个采样窗口内测得的原始输出速率(未平滑),`output_bytes` 是该窗格终端的累计计数。两者都用于调优和诊断。当新进程接管终端时 `output_bytes` 会重置,因此数值下降应视为重启,而不是负的工作量。

这是拉取而非推送:没有 activity 事件,因为把固定采样周期上连续变化的值变成事件,会得到一条随窗格喧闹程度而膨胀的事件流。请按需要的频率轮询 `pane.get` 或 `pane.list`。它同样不会被持久化——刚启动的服务器在接下来几次采样落地前会把所有窗格报告为静止状态,而从未产生输出的窗格根本没有 `activity` 对象。
...
`config` 和 `ui` 为 Herdr 自身的层保留,会以 `invalid_agent_view` 被拒绝。
设置成功会原子替换之前由 API 拥有的视图。该视图会持续到被 clear、被替换、所属插件被
disable、unlink 或 uninstall,或者服务器退出。需要持久行为的插件应把查询保存到
`HERDR_PLUGIN_STATE_DIR`,并从 `[[startup]]` 钩子重新应用。

有三个层可以拥有 Agents 视图,优先级为 `config` < `ui` < `plugin`。套接字 API 只
拥有最高的 `plugin` 层,因此 `agent.view.set` 始终会接管面板。`config` 层是声明式的
[`[ui.sidebar.agents.view]`](/docs/configuration/#智能体面板视图) 配置键,跨重启持久,
且不会被 API 调用覆盖或清除。`ui` 层保留给 Herdr 为自身设置的视图。
