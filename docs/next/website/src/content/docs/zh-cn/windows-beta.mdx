...
在 `herdr --remote` 期间,由 `keys.remote_image_paste` 设置的按键会从 Windows 剪贴板读取图像并传输到远程主机;远程端把它写入临时文件,并把该路径粘贴到当前聚焦的窗格。把一个本地图像文件拖放到终端上也走同一条传输路径。Windows Terminal 会占用默认绑定 `ctrl+v` 作为自己的粘贴,请改绑到它不会占用的按键,例如 `alt+v`。在没有远程主机可复制文件的本地原生 Windows 窗格中,剪贴板图像粘贴仍然未验证。
...
在 Windows 上进行远程工作,请从 Windows 二进制运行 `herdr --remote you@server` — 参见[持久化与远程访问](/zh-cn/docs/persistence-remote/)。远程服务器本身仍须是 Linux 或 macOS;Herdr 的服务端不在 Windows 上运行。
