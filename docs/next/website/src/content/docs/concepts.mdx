...
Herdr is mouse-native. You can click panes, tabs, workspaces, and agents. You can drag split borders, select text, right-click a pane to paste, and open Herdr's context menus with `shift`+right-click. Everything below is also reachable by mouse; keyboard bindings are an optional layer.
