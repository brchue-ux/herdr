...
During `herdr --remote`, the key set by `keys.remote_image_paste` reads an image off the Windows clipboard and transfers it to the remote host, which writes it to a temp file there and pastes that path into the focused pane. Dropping one local image file onto the terminal transfers that file the same way. Windows Terminal claims `ctrl+v` — the default binding — for its own paste, so rebind the trigger to something it does not take, such as `alt+v`. Clipboard image paste into a local native Windows pane, where there is no remote host to copy the file to, remains unverified.
...
Herdr's pane text copy works on Windows beta. Drag-select text inside a pane, then press Ctrl+C to copy it through Herdr; set `ui.copy_on_select = true` to copy on selection instead.

For text paste, right-click the pane, or use `ctrl+shift+v` in Windows Terminal. Multiline text paste is bracketed so shells and agent prompts receive it as one paste instead of submitting each line separately. Herdr's own context menu is on `shift`+right-click; if Windows Terminal keeps `shift`+mouse for its own bypass and never forwards it, use `ctrl`+right-click instead.
...
For remote work from Windows, run `herdr --remote you@server` from the Windows binary — see [Persistence and remote work](/docs/persistence-remote/). The remote server itself must still be Linux or macOS; Herdr's server does not run on Windows.
