...
- **Every paint reads a pane's live grid through one seam.** `GhosttyPaneTerminal::render` draws a whole pane and `ghostty_collect_dirty_patch` patches the retained frame the server sends instead of a full one, and both used to call `render_state.update(terminal)` for themselves — so a rule about *when* a pane may be read has to hold in both, or the retained fast path quietly reintroduces exactly what the full path refuses. `refresh_render_state` (`src/pane/terminal.rs`) is that one read, and it is where a pane holds its last complete frame while its child is inside a DEC 2026 synchronized update rather than painting a half-drawn grid. Three things about it are load-bearing. `render_state` is retained on the pane core, so *not* calling update is literally "draw the last complete frame" — no second copy exists or is needed. A held frame must not clear row dirty flags, because those flags are what makes the batch's final content reach the retained path once the batch closes. And the hold is bounded twice over — by `SYNCHRONIZED_UPDATE_HOLD_TIMEOUT`, so a child that raises mode 2026 and dies cannot freeze its pane, and by having never drawn a live frame yet, so a pane whose first output arrives inside a batch draws it rather than rendering blank.
