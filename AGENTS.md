...
- **The character tree is the layout authority, and a glyph's ink is not at its cell's edge.** `tree_prefix_width` (`src/ui/sidebar.rs`) is the single place a row's prefix is measured, and a card's left border deliberately stands in its connector's own column so the rail and the border share it. That works in characters because both are box-drawing verticals, and a font centres those in the cell so they can meet across rows. It does not survive being drawn in pixels: a rounded rect's stroke sits on `frame.x`, which is a cell *boundary*, so a pixel card's border landed half a column left of every rail meant to continue it — one offset, reported as two findings ("trunk not aligned with firstmate", "branches not aligned with secondmates"). `image_card::RAIL_INK_COLUMN_FRACTION` is where the pixel side is moved onto the character geometry, and it is the side that gives because a glyph goes where the font puts it. Anything new that draws a tree line in pixels aligns to the same fraction, not to the cell edge. Aligning the column used to be only half of it, and the other half is now gone: the sheet was opaque over every cell a row owned, so the two stretches of tree line crossing a card's own cells were painted over and `Rasteriser::draw_tree_joins` put them back in pixels. **Neither pixel model paints a backdrop any more** — a card is glass (see the glass-card bullet) — so the tree's line is simply on screen where the character renderer drew it, and both the joins and that function are deleted. What replaced them is the opposite rule: the vertical rail leaves a card's *bottom edge* and runs in the gutter, never through the card, and `ui::sidebar::no_rail_cell_lands_inside_a_pane_box` holds the overlap at exactly zero. Before believing any of this, note that a character-only test cannot see the *column* alignment at all — the card's border glyph fills the same column — so a change to that is still verified by reading the published sheet's pixels. Relatedly, **the cards are right-aligned**: every card's right edge is the panel's, and both of the questions a row answers are spent on its *left* edge through `card_left_offset` — `WorkspaceListEntry::depth` (where the row hangs) plus `WorkspaceListEntry::rank` (what the row is), which is what lets a worker the first mate opened sit on the first mate's branch without drawing at second-mate size. That replaced a symmetric layout that insetting the right edge by rank produced, on the captain's word.
- **The server's tick and the app's tick are two loops, and the server's is the only one a real Herdr runs.** `HeadlessServer::handle_scheduled_tasks_headless` is *not* a caller of `App::handle_scheduled_tasks`; it is a second, partial copy of it, and the comment on the tray's line in it says so. Anything periodic added to the app loop alone therefore never runs in production, and it fails **silently** — the machine register sat in that other loop for its whole life and was never sampled on any session, on any platform, while its corner reported *"the host's own state is not read on this platform"*. Two rounds of investigation went to a stale installed binary and the client/server transport before the missing call was found. When adding an `observe_*`, put it in both loops and write the test against the headless one (`the_headless_tick_samples_the_machine_register`). The second half of that defect is worth copying too: an absence enum whose "we never asked" case is spelled as its "the platform does not support this" case will send the next reader to the platform layer, so `machine_register::Absence::NeverSampled` is a distinct variant on purpose.
- **The sidebar card is glass, and the sidebar reads the sky's register.** Two changes that arrived together and are only sensible together. *(a)* A card is a **translucent pane, not a filled plate**: a face at `measured::GLASS_FACE_ALPHA` (a tenth), a second face offset `GLASS_THICKNESS_PX` down-right for thickness, sharp corners capped at `RADIUS_MAX_PX`, a bright edge, and **no backdrop on either pixel model** — so the whole-terminal background scene is measurably visible through every card, which is the point. `shape_covers_row` therefore answers `true` for the sheet as well as for shapes: nothing is painted over the character card, so the character card must stand down under both. The gates are `what_is_behind_a_card_reaches_through_its_face` (a difference test, not a threshold — an occluding card gives the same answer whatever its alpha claims) and `the_trees_ink_is_one_hue_family_and_replaces_little_of_what_is_behind_it`. *(b)* A Space's row says what its **body** is, in the register `src/solar_system.rs` draws it in — `gas giant · 99 files · 2 moons` and `streak 5 · T 13.4s · 23 revs`. `src/ui/sidebar/body_register.rs` owns that and **reads the scene's own functions rather than re-deriving them** (`seat_the_ladder`, `assign_body_types`, `BodyKind::revolutions_per_loop`, `OrbitTracks::revolutions`): A54's *"body type is theirs and is binding"* cannot be binding if two modules each decide it. It is resolved **once per pass** and held across every per-row loop — ranking is `O(n log n)` over the roster and a per-row `resolve` makes the panel `O(n² log n)` in rows on screen. Two consequences worth knowing before touching either: a card carries exactly `CAPTION_LINES` captions (two) and which two depends on whether the row is a mate or a worker, and dropping `branch`/`git_status` from the default Space rows correctly stops the background git refresh for a fleet that names neither — `git_refresh_demand` is derived from the configured rows and that is deliberate, so a test needing a refresh has to configure a git token.
- **A row arrives in place, never sideways.** `motion::ArrivalBeat` is four beats: a light down the tree's rail, right along the elbow, the card **generated** left-to-right from where it landed (`CardContent::generate`, a clip in `draw_card`), and the column below pushed down throughout. `row_offsets`'s `dx` is now always exactly `0.0` and `no_row_ever_carries_a_horizontal_entry_offset` sweeps the whole arrival to prove it — the old slide was zero at both ends and a panel width across in the middle, so an endpoint check saw nothing. The load-bearing consequence is elsewhere: "is this row mid-transition" used to be spelled `motion_cells.0 != 0`, and with the slide gone that proxy is dead. It is `WorkspaceCardArea::arriving` now, stamped in `ui::update_sidebar_card_layers` and read by `WorkspaceCardArea::drawn`; without it a departing row keeps owning its slot while the row contracting over it is drawn there, and every click aimed at the visible row is swallowed by the invisible one.
- **There is one agent-state alphabet and it is configurable.** Every surface — sidebar, tabs, navigator, mobile, worker summaries — draws its state glyph through `ui::status::state_icon`/`state_icon_symbol`, dispatched on `ui.status_indicators`. This fork ships a third variant, `ascii` (`!` blocked, `>` working, `-` idle, blank unknown); upstream's `dots` and `symbols` remain selectable. `dots` became the shipped default from 2026-08-21 to match the flight-deck mockup's solid status dot — know before touching this again that its three glyphs (`●`/`○`/`·`) are *all* East-Asian Ambiguous width and `·` collides with the sidebar's own token separator, the exact two failure modes `state_mark`'s doc comment documents as why `ascii` was made default in the first place; that trade was made knowingly for this mockup match, not rediscovered. Adding a surface that calls `state_mark` directly, or a variant without extending every `match` on `StatusIndicatorStyle`, is how the alphabet drifts.
...
- **Stage and severity stay independent whether a surface uses many hues or one.** On an opt-in split-hue card, hue carries [`LifecycleStage`](src/anim/cell.rs), intensity carries [`Severity`], and both meet in `anim::cell::signal_ink`. The orrery's planets/moons (`solar_system.rs`) keep one material hue family driven by stage: `anim::cell::one_hue_stage_mix(stage)` moves it onto saturation/luminance, while severity keeps the independent lightness ramp. The sky keeps the artifact's six warm planet albedos and four near-neutral icy moon albedos; lifecycle never chooses among them. **The sidebar's default (non-split) pixel-card border no longer reads `stage` at all** — `image_card::CardLight::of` now calls `one_hue_stage_mix` with a binary `accented` instead (`CardContent::accented`: the focused Space, or a card mid its own arrival bloom, `generate < 1.0` — nothing else), after the captain's `herdr-card-border-dot-final-match-20260822` fix found every `working`/`blocked`/`waiting` card drawing the mockup's strong accent for its entire life. A card's `AgentState`/severity now only reaches the badge, the chip, the discharge filaments and the worker's `.wk-dot` (`is_worker`, read off `register`'s `CaptionTone::State` — no dedicated field) — never the border or (dormant, see below) the outer bloom. Do not restore a `stage`-keyed border without re-reading that task's screenshots. Intensity is placed as a **fraction of the lightness headroom between the panel and the end of the scale**, and the obvious alternative was measured and rejected twice: normalising to a WCAG *contrast ratio* is unreachable across hues without bleaching them (pure blue tops out at 2.2:1 on a near-black panel where pure green reaches 13.9:1), and a *saturation* ramp raises a colour's maximum channel, which is louder on a dark theme and quieter on a light one, so a luminance-heavy hue stops being four steps at all. Two consequences to keep: an ink must never be produced by mixing toward a *coloured* surface, because only a mix toward pure black or pure white preserves a hue exactly; and severity also picks the escalated `card-alert` rung of the breath ladder, because the visual target is explicit that a state has to read for someone who cannot separate two hues. The stage and the severity are read from the reserved published tokens `lifecycle` and `severity` in `src/app/lifecycle.rs`, the same way `owner` is read, with detection as the fallback — Herdr can see four of the five stages itself and can never see the fifth, since an idle prompt after a failure is byte-identical to one after a success.
...
- **A live, already-running server cannot be profiled with `HERDR_RENDER_PROF` after the fact.** `render_prof::enabled()` (`src/render_prof.rs`) reads the env var into a `OnceLock` on first use, so it is fixed for the process's whole life; there is no signal or runtime toggle, only a restart with the var set — which a live captain session cannot be given without the same restart risk as any other server-global operation. When a live server's CPU is high and the cause is unclear, `/proc/<pid>/stat` (utime+stime delta over a few seconds) and `ps -T -p <pid>` are read-only and safe against the real process; they cannot name a function, but the *shape* is diagnostic on this codebase's threading model: cost concentrated on the single main/event-loop thread while `tokio-runtime-w*`, `herdr-pty*`, and the card-raster thread pool sit near zero points at the synchronous scheduled-task/render path (unrelated to card rasterisation, which fans out over `std::thread::scope`), whereas cost spread across many threads points at PTY throughput. `/proc/<pid>/io`'s `rchar`/`syscr` delta separates the two further: high bytes-per-read is real output volume, many tiny reads (double-digit bytes each) at a high per-second rate is frequent small pane/status updates driving repeated render passes rather than a data-volume problem. `perf` may be blocked by the host's `perf_event_paranoid`; treat that as a per-machine fact, not a codebase one. A live-handoff swap does not carry the swapping shell's env into the new process either — `HERDR_RENDER_PROF` was absent from the post-handoff process's `/proc/<pid>/environ` even though exported before the swap — so arming it on a live session needs a full cold restart, not a handoff. Before suspecting a hidden tight poll loop for a high syscall rate: `src/pty/actor/unix.rs`'s per-pane reader blocks in `poll()` (1000ms idle timeout, not a spin), `src/app/pane_activity.rs`'s sampler is capped at 250ms by design specifically so busy output can't raise the loop's wake rate, and `src/server/client_transport.rs`/`src/protocol/wire.rs::read_message` are genuinely blocking reads with no retry-sleep. The one real `Duration::from_millis(1)` in production code, `STREAM_FALLBACK_POLL_INTERVAL` in `src/api/server/pane_graphics_stream.rs`, only runs when the socket's `recv_timeout` is `Unsupported` — `#[cfg(not(windows))]` never takes that branch, so it is inert on Linux/macOS.
- **The background scene has a design authority outside this repo, and it keeps moving.** `src/solar_system.rs` is a separately-implemented Rust build of the fleet-orrery concept whose blueprint is the `Skill_and_Direction_v1` repo's `examples/fleet-orrery/commitment.md` (the settled decisions, amended in numbered sets — A38 mass register, A41/A42 the ladder, A51 body surfaces, H6 the starfield) and `examples/fleet-orrery/hone-bodies.html` (what is actually built there). Herdr is expected to *match its decisions*, never to embed or run it, and the module's own constants — `FILE_FLOOR`, `FILES_CEIL`, `TERMINATOR_WIDTH`, `STAR_CLASS` — are ported from that card rather than re-derived, so a project reads the same size and a body the same surface in both places. Two things follow. Do not retune one of those constants without going back to the clause it came from; and when reconciling, read the card rather than the artifact's rendered output, because the card marks which decisions are settled and which are still open (an open one is the captain's, not the implementer's). Herdr's own captain rulings live in the firstmate home's `data/decisions/`, and where the two disagree the ruling wins and the module's doc comment says so.
...
- **"The background scene is not drawing" has an authoritative answer, and it is not config.** The scene is gated on a conjunction whose parts fail *silently and independently* (`AppState::background_scene_active`): two `[experimental]` keys that must agree, plus whether the terminal on the other end of the pty is one Herdr positively identified as drawing an opaque wash *under* the text — `HostTerminalKind::draws_ambient_wash`, which cannot be probed and so refuses anything it cannot name. Ask `herdr background status` (`src/cli/background.rs`) rather than re-deriving it; it prints every condition and is fed by `background_scene` on the session snapshot, read from the same predicate the renderer reads. Two things that mislead here. The terminal identity comes from XTVERSION in band over the pty (`src/host_terminal_identity.rs`), *not* from `TERM_PROGRAM` — which is why it survives an SSH hop and why `host_terminal: other` means the terminal did not answer or gave an unknown name, never that an env var was unset. And the wash is withheld from *every* viewer if any one attached viewer would composite it over its own text (`every_app_viewer_draws_ambient_wash`), so a second client on an unidentified terminal blanks the scene for a first client that would have drawn it fine.
...
- **The Agents view is a filter over the same one tree, and it can empty the panel completely.** `app/agent_view.rs::apply_agent_view` retains `AgentPanelEntry`s against the active view before `arrange_agent_tree` runs, so a view is not a second panel — it removes rows from the tree everything else is measured against. The captain hit the pathological case: a durable `exists owner` view (the API tier is the one that persists to the session file and is reapplied at startup) plus a fleet where nothing published an `owner` token, which filtered out every worker and left a panel indistinguishable from an idle fleet. `AgentViewHidden` counts exactly that, and the tree header now draws it (`tree_view_notice_label`), so a filtered-empty panel names the view rather than reading as an outage. Two things follow. The count is resolved once in `compute_view` and parked on `ViewState::sidebar_view_hidden` — the header row is explicitly not allowed to walk the fleet a second time per frame, and resolving it walks every pane of every tab of every Space. And Spaces are *not* filtered by a view: a mate draws from worktree membership alone, so "the mates vanished" is never this, while "the workers vanished" usually is.
...
- **Every read of a pane's live grid goes through one seam — paints *and* non-paints.** `GhosttyPaneTerminal::render` draws a whole pane and `ghostty_collect_dirty_patch` patches the retained frame the server sends instead of a full one, and both used to call `render_state.update(terminal)` for themselves — so a rule about *when* a pane may be read has to hold in both, or the retained fast path quietly reintroduces exactly what the full path refuses. `refresh_render_state` (`src/pane/terminal.rs`) is that one read, and it is where a pane holds its last complete frame — through a DEC 2026 synchronized update, through a resize's own redraw, through a redraw on a pane that has been hidden, and through an alternate-screen history harvest — rather than painting a half-drawn or mid-scroll grid. Three things about it are load-bearing. `render_state` is retained on the pane core, so *not* calling update is literally "draw the last complete frame" — no second copy exists or is needed. A held frame must not clear row dirty flags, because those flags are what makes the batch's final content reach the retained path once the batch closes. And every hold is bounded, so a child that raises mode 2026 and dies, or a harvest whose release never arrives, cannot freeze its pane. **The seam is only a seam if nothing else advances `render_state`, and for six shipped fixes nothing enforced that.** The per-frame hyperlink scan (`render_stream::visible_hyperlinks`, run over every visible pane immediately after it is drawn, once per full render per client), the pane-read API's `ghostty_visible_text`, `palette_color_query_response`, and — on Windows, on every PTY write — `windows_recent_fallback::visible_render_lines` each called `render_state.update` directly. Every hold therefore released itself on the very next frame no matter what its own timeout said: five PRs (#169/#172/#174/#175/#177/#182) each correctly widened *when* a hold arms and none of them held anything, which is why the captain's scroll-then-snap survived all of them and why every unit test still passed — a pane-level test calls `render` twice with nothing in between, and the live frame loop does not. The fix is `RenderStateUse`: a `Present` refresh stamps the presentation clock and clears a resolved redraw hold, a `Read` refresh sees exactly what the pane is currently showing and touches no hold state (a poller reading a backgrounded pane must not make it look freshly drawn, or the hidden-pane arming site never fires). **Anything new that needs a pane's cells calls `refresh_render_state_for(.., RenderStateUse::Read)`, never `render_state.update`** — and note that a test which cannot fail against the old code is the normal outcome here, so verify a regression test by reverting the production change and watching it fail.
...
- **Box drawing and the block elements are computed from the cell, never taken from the face.** `src/grid_raster/cell_shapes.rs` maps `U+2500..=U+259F` to rectangles in cell pixel coordinates, and `rasterise_region_with_font` prefers that over the glyph for any single-character cell it covers. This is not a refinement — it is what makes the pixel path usable at all. A face's glyphs are sized from its own em box and then fitted into the host's cell (`GridFont::fit_to_cell`), so ink lands *inside* the cell with a margin, which is right for a letter and wrong for a rule: measured live under kitty at a 10x21 cell, `█` came out 19px tall, so a wall of blocks was striped with 2px seams and every pane border broke into a dashed line at each cell boundary. The rule to keep is that a shape's rectangles run to `cell_width`/`cell_height` exactly and are clipped to their own cell (unlike a glyph, which is clipped to the whole surface so descenders and double-width halves can overhang) — that exactness is the whole mechanism by which two neighbouring cells join. The dashes, doubles, arcs and diagonals deliberately return `None` and keep the font path; each needs geometry a rectangle list cannot express, and they are the named next slice rather than an oversight.
...
- **The pane runtime size is the one member of that family that cannot be folded, so it is *reported* instead.** `effective_size` is the foreground client's size and it sizes the shared PTY grids, which is a real resource with exactly one size — there is no "withdraw to a safe default" available the way there is for a raster. Every other client therefore renders through `ui::compute_view_without_resizing_panes`, computing pane rects at *its own* area around grids that are still the foreground client's size: a short block of real output above a blank tail, or output clipped off the bottom, persisting until one of them detaches. That is deliberate — a spectator must not fight the primary client for pane sizing — so the guard is not to change the sizing but to stop it being silent. `AppState::pane_size_pin` is set by `render_and_stream` for the duration of that one client's draw and cleared after (the same set/restore the scroll offsets beside it already use), and `ui::status::render_pane_size_pin` draws it in the config diagnostic's own top-right banner. Two things to keep true when touching it: the notice must never outlive the client it was set for, and `ui::warning_banner_present` must stay the single predicate for "a banner is drawn", because `compute_view` uses it for the toast's hit area while `render_notifications` uses it for where the toast is drawn.
- **Moving a surface's drawing to the client is four joints, and three of them are easy to miss.** Three surfaces work this way — `ServerMessage::CardScene` (`src/ui/sidebar/image_card.rs`), `ServerMessage::TrayScene` (`src/ui/sidebar/tray.rs`), and `ServerMessage::BackgroundScene` (`src/app/background_scene.rs`, wrapping `solar_system::SceneLayout`/`SceneEffects`) — and all three ship *semantic tokens*, never pixels; the client rasterises with the same unchanged function the server would have used, against its own cell size, which is deliberately not on the wire. (1) The variant is **appended at the end** of `ServerMessage`, per the warning in `src/protocol/wire.rs`; anywhere else shifts the wire tag of everything below it. (2) The pixels must be withheld from that client's own pass — `EmbeddedSurfaces` in `src/kitty_graphics.rs`, one flag per negotiable surface, built from the connection by `ClientConnection::embedded_surfaces`. (3) The client needs a **separate `HostGraphicsCache` per scene surface**: `encode_graphics_update` deletes the layer images of every source absent from the pass it is handed, so one cache shared across two separately-encoded surfaces has each surface delete the other on every frame. Its pending bytes are the encoder's *deltas* against that cache, so an unflushed batch must be appended to, never replaced, or the cache believes in an upload that never went out. (4) Withholding pixels is not the same as not computing them — `include_cards` and its `EmbeddedSurfaces` successor only stop the *shipping*, so a server that withholds a surface goes on drawing and PNG-encoding it for nobody. All three now stop drawing too, and each needs the same two supporting parts: the skip is gated on a unanimous `every_app_viewer_rasterizes_*` fold (`src/server/headless.rs`) because the artwork is shared `AppState` that every viewer is placed a copy of, and an `AppState::*_client_rasterized` flag tells the character/server path to stand down anyway, because the surface is coming — just not from here. Tray: `every_app_viewer_rasterizes_signal_tray` + `signal_tray_graphics_client_rasterized`, read by `tray::render`; a badge is mostly transparent, so a mark left under one shows *through* it. Cards: `every_app_viewer_rasterizes_sidebar_cards` + `sidebar_card_graphics_client_rasterized`, read by `ui::update_sidebar_card_layers`, which still runs `compute_card_placement` (the cheap half — it stamps each row's motion offset for the character connectors, and its failing is the honest way to say the client will have no cards either) and reports `CardsUpdate::Delegated`. That is a fourth variant rather than a reuse of `Empty` precisely because "nothing was drawn here" and "there is nothing to draw" must not read the same: a shape is transparent outside its own glow, so an `Empty` reading leaves the character card showing through the client's own artwork. Background scene: `every_app_viewer_rasterizes_background_scene` + `background_scene_client_rasterized`, read by `App::observe_background_scene`/`observe_background_effects` — the one surface here where the server-side generation being skipped is not a cheap per-frame raster but the periodic `herdr-scene-bake` thread's whole `solar_system::FRAME_COUNT`-frame PNG loop plus the per-tick synchronous effects overlay, which is exactly the cost the fix in `data/decisions/2026-08-06-r1a-commit.md` (firstmate home) was written against: it used to run on the shared Linux server's own CPU regardless of how idle a delegating Windows client's machine was. The layout still installs every pass rather than waiting on a completed bake, since a delegating client has no baked image left to stay in sync with. The check that actually proves such a move is byte-equality of the emitted Kitty payload, not of the pixels (`a_client_rasterised_tray_emits_the_bytes_the_server_would_have_embedded`): identical pixels at a different image id, placement id or rect still land in the wrong place. All three surfaces are gated on `cfg!(windows) && is_remote_client_process()` with a `HERDR_CLIENT_RASTERIZED_*` env override, because there is no Windows hardware to exercise the real gate from a Unix box. That platform check alone is a "would I", not a "could I" — it says nothing about whether this process's own `kitty_graphics` is even on, which independently gates whether the client ever asks its terminal the capability question (`query_kitty_graphics_capability`) and whether it reports a real cell size at all (`0x0` when off, `initial_terminal_geometry`). `do_handshake` therefore also requires `may_claim_client_rasterized_surfaces(kitty_graphics_enabled)` before honoring any of the three `wants_client_rasterized_*` answers in the Hello — a client that claims delegation it cannot deliver leaves the server standing its own rendering down for a surface that then never arrives from either side, permanently, with `background_scene.active: true` and zero fallback. Confirmed live against the captain's own `herdr-server.log`: every `--remote` connection that night negotiated `TerminalAnsi` with `cell_width_px=0`/`cell_height_px=0`. Note what has *not* moved: the animation clock. `breath`, a badge's `motion` envelope, and the background scene's `phase` are all resolved server-side and shipped already-stepped, so the drawing is on the client and the clock driving it is not.
...
A red `Windows ConPTY package` or `live compositing` is usually a *build* failure, not a test failure, and usually not yours. Both build the vendored libghostty-vt through `zig build`, which resolves `vaxis`'s own transitive packages (`zigimg`, `uucode`, `imgui`) over the network at build time, so any github.com hiccup fails them with `error: invalid HTTP response: HttpConnectionClosing` panicking out of `build.rs` — an error that looks nothing like the diff that triggered it. Before attributing one to a pull request, check whether the same job failed on `master` around the same time; when it did, `gh run rerun <run-id> --failed` is the whole fix. The lever that decides how exposed a job is, is whether it restores `.zig-cache`: every Windows job now does, and `windows-conpty-package` was flaking three runs in four until it got the same cache step `check (windows-latest)` already had. A new job that builds Herdr and skips that step is opting back into the same failure.
...
Neither rig covers the `--remote` bridge's sidebar or background scene, which
is a different drawing path: a delegating client is sent
`CardScene`/`TrayScene`/`BackgroundScene` tokens and rasterises the pixels
itself, so a tap of the *server* socket sees no graphics at all.
`HERDR_CLIENT_RASTERIZED_CARDS=1`, `HERDR_CLIENT_RASTERIZED_SIGNAL_TRAY=1`, and
`HERDR_CLIENT_RASTERIZED_BACKGROUND_SCENE=1` put a Unix client on that path
...
`wants_client_rasterized_signal_tray` / `wants_client_rasterized_background_scene`
are all `cfg!(windows) && is_remote_client_process()`, so a Windows `--remote`
client is sent `CardScene`/`TrayScene`/`BackgroundScene` **tokens** and
rasterises the sidebar and the whole-terminal ambient scene itself instead of
receiving pixels. Several whole classes of bug reach only that population.
...
- This fork's `HERDR_INTEGRATION_VERSION` numbers are a **fork-owned line kept strictly above upstream's** for every asset it has absorbed, not a continuation of upstream's. `integration_status_at` (`src/integration/registry.rs`) calls an install Current only when the marker on disk is `>=` the bundled constant, so equal numbers with different content are the failure mode: whichever binary wrote the hook first stays in place forever, because neither side ever sees the other as newer. When absorbing an upstream commit that bumps an asset, set the new value above **both** the fork's previous value and upstream's current value, and update the asset marker and its `*_INTEGRATION_VERSION` constant together — `bundled_integration_asset_versions_match_expected_versions` enforces that pairing.
...
- When changing the server/client wire protocol, compare `src/protocol/wire.rs::PROTOCOL_VERSION` against protocols published in both stable and preview releases. Bump it when the current source protocol has already been published in either channel and the wire format changes incompatibly. Do not bump it again for multiple incompatible changes before that protocol is published. Update hardcoded protocol expectations and manual protocol fixtures in tests: `tests/support/mod.rs::CURRENT_PROTOCOL`, `tests/api_ping.rs`, `tests/cli/sessions.rs`, `docs/next/api/herdr-api.schema.json` (regenerate with `HERDR_UPDATE_API_SCHEMA=1 just test-one generated_protocol_schema_artifact_is_current`), and the hand-rolled bincode-varint `ClientMessage::Hello` encoders in `tests/cross_area.rs`, `tests/server_headless.rs`, `tests/multi_client.rs`. `#[serde(default)]` on a new struct field is not itself skew-safe against bincode's positional (non-self-describing) encoding: the decoder only "runs out of bytes" to trigger the default for a field truly at the very end of the whole message, never for one nested inside a `Vec` of more than one element or followed by further sibling fields — that shape reliably raises a hard decode error instead (confirmed for `CardContentWire::focused_space` inside `CardScene`, see PR reserving PROTOCOL_VERSION 24). `PROTOCOL_VERSION` is what actually protects such a change; do not rely on `#[serde(default)]` alone to justify skipping the bump.
- The `check` job in `.github/workflows/ci.yml` deliberately runs only `ubuntu-latest` and `windows-latest`. The macOS test leg was dropped and is only reconsidered if the captain explicitly says he has bought a Mac; see the comment at that point in `ci.yml`. The macOS *build* jobs in the release/preview/dev-binary workflows are unaffected.
