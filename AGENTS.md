...
A red `Windows ConPTY package` or `live compositing` is usually a *build* failure, not a test failure, and usually not yours. Both build the vendored libghostty-vt through `zig build`, which resolves `vaxis`'s own transitive packages (`zigimg`, `uucode`, `imgui`) over the network at build time, and neither job restores a `.zig-cache` — `windows-conpty-package` sets `use-cache: false` and adds no `actions/cache` step at all, so it cold-fetches on every single run. Any github.com hiccup then fails them with `error: invalid HTTP response: HttpConnectionClosing` panicking out of `build.rs`, which looks nothing like the diff that triggered it. Before attributing it to a pull request, check whether the same job failed on `master` around the same time; when it did, `gh run rerun <run-id> --failed` is the whole fix.
